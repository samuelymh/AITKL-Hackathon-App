...
async function testEndpointPerformance(endpoint, description, expectedItems = 10) {
  ...
  try {
    ...
    const itemCount =
      result.data?.data?.length || result.data?.data?.grants?.length || result.data?.data?.prescriptions?.length || 0;
...
async function runPerformanceTests() {
  ...
  for (const test of tests) {
    const result = await testEndpointPerformance(test.endpoint, test.description, test.expectedItems);
  ...
  results.forEach((result) => {
    const status = result.avgTimePerItem > 50 ? "⚠️" : result.avgTimePerItem > 20 ? "⚡" : "✅";
