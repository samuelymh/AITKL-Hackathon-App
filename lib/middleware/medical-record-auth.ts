...
export class MedicalRecordAuthMiddleware {
    ...
    return async (
      ...
      try {
        ...
        const { patientId, organizationId } =
          await this.extractAuthParams(request);
  ...
  private static async extractAuthParams(request: NextRequest): Promise<{
    ...
    try {
      ...
      const patientId =
        url.searchParams.get("patientId") || url.searchParams.get("userId");
...
export const withViewMedicalHistory = (handler: any) =>
  MedicalRecordAuthMiddleware.withMedicalRecordAuth(
    handler,
    "viewMedicalHistory",
  );
