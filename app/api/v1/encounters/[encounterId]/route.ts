...
export const GET = withEncounterAuth(
  async (request, { params }, authorizedContext) => {
    await dbConnect();
    ...
    await EncounterAuthMiddleware.validateEncounterAccess(
      encounterId,
      authorizedContext,
    );
 ...
 * Update an encounter
 */
export const PUT = withEncounterAuthAndBody(
  async (request, { params }, authorizedContext, body) => {
    await dbConnect();
