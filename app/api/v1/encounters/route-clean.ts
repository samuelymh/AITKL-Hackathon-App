 ...
 */
export const GET = withEncounterAuth(async (request, context, authorizedContext) => {
  await dbConnect();
 ...
 */
export const POST = withEncounterAuthAndBody(async (request, context, authorizedContext, body) => {
  await dbConnect();
  ...
  const encounter = await EncounterService.createEncounter(body, practitionerId, authorizationGrantId);
