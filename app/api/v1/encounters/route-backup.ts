...
export const GET = withEncounterAuth(
  async (request, context, authorizedContext) => {
    ...
    const organizationId = authorizedContext.authGrant.organizationId._id.toString();

    // Use service for business logic
    const { encounters, totalCount } = await EncounterService.listEncounters(
      userId,
      organizationId,
      query,
...
export const POST = withEncounterAuthAndBody(
  async (request, context, authorizedContext, body) => {
    ...
    await EncounterService.validateReferencedEntities({
      userId: encounterData.userId,
      organizationId: encounterData.organizationId,
      practitionerId: encounterData.practitionerId,
    });

    // Create the encounter using service
    const savedEncounter = await EncounterService.createEncounter(
      encounterData,
      practitionerId,
      authorizedContext.authGrant._id.toString()
    );

    // Populate for response
    const populatedEncounter = await EncounterService.getEncounterById(savedEncounter._id.toString());
    ...
    auditLogger.log({
      ...
      details: {
        ...
        diagnosesCount: savedEncounter.diagnoses.length,
        prescriptionsCount: savedEncounter.prescriptions.length,
      },
    });
