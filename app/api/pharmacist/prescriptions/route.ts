 ...
 */
async function getPharmacistPrescriptionsHandler(request: NextRequest, authContext: any) {
  try {
    ...
    if (status === "pending") {
      prescriptions = await getPendingPrescriptionVerifications(practitioner._id.toString());
    } else {
      prescriptions = await getRecentPrescriptions(practitioner._id.toString(), limit);
    }
    ...
    return NextResponse.json({ error: "Failed to fetch pharmacist prescriptions" }, { status: 500 });
  }
}
