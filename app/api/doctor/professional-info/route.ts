    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to get professional information",
      },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!allowedRoles.includes(decoded.role as any)) {
      return NextResponse.json(
        {
          success: false,
          error: "Access denied - healthcare provider role required",
        },
        { status: 403 },
      );
    }
    ...
    const result = await executeDatabaseOperation(async () => {
      return await updatePractitionerInfo(
        decoded.userId,
        decoded.role,
        professionalData,
      );
...
async function updatePractitionerInfo(
  userId: string,
  userRole: string,
  ...
  if (!practitioner) {
    practitioner = await createNewPractitioner(
      userId,
      userRole,
      professionalData,
    );
  } else {
    practitioner = await updateExistingPractitioner(
      practitioner,
      professionalData,
    );
  }
...
async function updateExistingPractitioner(
  practitioner: any,
  professionalData: any,
) {
...
async function updatePractitionerFields(
  practitioner: any,
  professionalData: any,
) {
  const updates = {
...
async function handleOrganizationMembership(
  practitioner: any,
  professionalData: any,
) {
  if (!professionalData.organizationId) {
  ...
  const membershipData = {
    ...
    membershipDetails: {
      role:
        professionalData.practitionerType ||
        practitioner.professionalInfo.practitionerType,
      accessLevel: "limited",
      department: professionalData.department,
      position: professionalData.currentPosition,
