...
import { type PrescriptionMedication, type PatientInfo, type DispenseRequest } from "@/lib/types/pharmacist";
import { PrescriptionStatus, DispensationStatus } from "@/lib/constants";
import { pharmacistAPI } from "@/lib/services/pharmacist-api";
...
export default function ViewMedications({ digitalId, onBack, onDispensationSuccess }: ViewMedicationsProps) {
  ...
  const fetchMedications = async () => {
    try {
      const data = await pharmacistAPI.getPatientMedications(digitalId);
      setMedications(data.data.medications);
      setPatient(data.data.patient);
    } catch (error) {
      console.error("Error fetching medications:", error);
      toast.error(error instanceof Error ? error.message : "Failed to fetch medications");
    } finally {
      setLoading(false);
    }
  };

  const handleDispense = async (medication: PrescriptionMedication) => {
    ...
    try {
      ...
      const data = await pharmacistAPI.dispenseMedication(digitalId, dispenseRequest);
      ...
      toast.error(error instanceof Error ? error.message : "Failed to dispense prescription");
    } finally {
      setDispensing(null);
    }
  };
            ...
            <Card key={medication.id} className="border-l-4 border-l-blue-500">
              ...
              <CardContent className="pt-0">
                <div className="space-y-3">
                  ...
                  {/* Dispensation Info */}
                  {medication.dispensationStatus === DispensationStatus.DISPENSED && medication.dispensedAt && (
                  ...
                  <div className="flex justify-end pt-3">
                    {medication.canDispense ? (
                      ...
                      <div className="flex items-center text-sm text-gray-500">
                        {medication.dispensationStatus === DispensationStatus.DISPENSED ? (
                          <>
